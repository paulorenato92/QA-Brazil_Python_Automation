...
class UrbanRoutesPage:
    ...
    PHONE_NUMBER_CONTROL = (By.XPATH,
                            '//div[@class="np-button" or contains(@class, "np-button")]/../div[contains(@class, "np-text")]')
    ...
    BLANKET_OPTION_INPUT = (By.XPATH,
                            '//div[@class="r-sw-label" and contains(text(), "Manta")]/..//input[@class="switch-input"]')

    ICE_CREAM_PLUS_BUTTON = (By.XPATH,
                             '//div[@class="r-counter-label" and contains(text(), "Sorvete")]/..//div[@class="counter-plus"]')
    ICE_CREAM_VALUE = (By.XPATH,
                       '//div[@class="r-counter-label" and contains(text(), "Sorvete")]/..//div[@class="counter-value"]')
    ...
    # === SELETORES REAIS DO SEU DEVTOOLS ATUALIZADOS AQUI ===
    DRIVER_NAME = (By.CSS_SELECTOR, '.order-btn-group > .order-button > div:last-child')
    DRIVER_RATING = (By.CLASS_NAME, 'order-btn-rating')
    DRIVER_IMAGE = (By.CSS_SELECTOR, '.order-btn-rating ~ img')
    ...
    def set_from_field(self, address_from):
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(self.CAMPO_DE)).send_keys(address_from)

    def get_to(self):
        return self.driver.find_element(*self.CAMPO_PARA).get_property('value')

    def set_to_field(self, address_to):
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(self.CAMPO_PARA)).send_keys(address_to)
    ...
    def click_taxi_button(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.TAXI_BUTTON)).click()

    def click_comfort_plan_card(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.CARD_COMFORT_PLAN)).click()
    ...
    def get_current_selected_plan(self):
        return WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(self.SELECTED_COMFORT_PLAN)).text
    ...
    def set_card(self, card_number, card_code):
        ...
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.CARD_SUBMIT_BUTTON)).click()
    ...
    def set_message_for_driver(self, message):
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(self.COMMENT_FOR_DRIVER_INPUT)).send_keys(message)
    ...
    def click_blanket_and_handkerchiefs_option(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.BLANKET_OPTION_CHECKBOX)).click()

    def get_blanket_and_handkerchiefs_option_checked(self):
        return self.driver.find_element(*self.BLANKET_OPTION_INPUT).get_property('checked')
    ...
    def click_order_taxi_buton(self):
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.ORDER_TAXI_FINAL_BUTTON)).click()
    ...
    # === MÉTODO CORRIGIDO COM ESPERAS ATIVAS ===
    def get_driver_info(self):
        WebDriverWait(self.driver, 10).until(
            EC.visibility_of_element_located(self.DRIVER_RATING)
        )
        name = self.driver.find_element(*self.DRIVER_NAME).text
        rating = self.driver.find_element(*self.DRIVER_RATING).text
        image = self.driver.find_element(*self.DRIVER_IMAGE).get_attribute('src')
